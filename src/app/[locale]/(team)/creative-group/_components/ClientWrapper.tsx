...
export default function ClientWrapper({ data }: any) {
    ...
    return (
        <motion.main
            ...
            className="flex min-h-screen w-full flex-col gap-10 px-6 pb-10 pt-5 text-white md:px-7 md:pb-20 md:pt-10 lg:px-20"
        >
            <h1
            ...
            <motion.div className="grid h-auto w-full grid-cols-1 gap-10 md:grid-cols-3 md:gap-y-12 lg:grid-cols-4">
                {data.map((item: any, index: number) => (
                    <motion.div
                        ...
                        key={index}
                        className="group relative z-10 h-[300px] w-full cursor-pointer overflow-hidden rounded-[6px] transition-all duration-300 ease-in-out md:hover:z-10 md:hover:scale-110"
                    >
